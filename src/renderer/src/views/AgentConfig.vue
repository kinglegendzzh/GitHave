      ...
      <v-card-text>
        <v-form ref="form" v-model="valid">
          <v-expansion-panels multiple>
            ...
            <v-expansion-panel>
              <v-expansion-panel-header>基本设置</v-expansion-panel-header>
            ...
            <!-- 仓库配置 -->
            <v-expansion-panel>
              <v-expansion-panel-header>监听的代码仓库</v-expansion-panel-header>
              <v-expansion-panel-content>
                <!-- 已配置仓库列表 -->
                <v-row>
                  <v-col
            ...
            <!-- 其它设定略：AI评价语气风格设定等 -->
            <v-expansion-panel>
              <v-expansion-panel-header>AI评价语气风格设定</v-expansion-panel-header>
              <v-expansion-panel-content>
                <mode-pie-chart
                    title="AI评价语气风格设定"
                    :mode-data="config.mode_ranges"
                    @update:mode-data="updateModeRanges"
                ></mode-pie-chart>
                <!-- 新增：语气设置编辑区域 -->
            ...
            <v-expansion-panel>
              <v-expansion-panel-header>AI消息总结语气风格设定</v-expansion-panel-header>
              <v-expansion-panel-content>
                <mode-pie-chart
                    title="AI消息总结语气风格设定"
                    :mode-data="config.mode_ranges_second"
                    :color-palette="['#4CAF50', '#2196F3', '#FFC107', '#E91E63', '#9C27B0', '#607D8B']"
                    @update:mode-data="updateModeRangesSecond"
                ></mode-pie-chart>
                <!-- 新增：语气设置编辑区域 -->
                <v-divider class="my-3"></v-divider>
                <div>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="repoForm">
            <v-text-field
                label="名称"
                v-model="selectedRepo.Name"
                outlined
            ></v-text-field>
            <v-text-field
                label="代码地址"
                v-model="selectedRepo.RepoURL"
                outlined
            ></v-text-field>
            <v-text-field
                label="用户名"
                v-model="selectedRepo.Username"
                outlined
            ></v-text-field>
            <v-text-field
                label="密码"
                v-model="selectedRepo.Password"
                outlined
                type="password"
            ></v-text-field>
            <v-text-field
                label="分支"
                v-model="selectedRepo.Branch"
                outlined
            ></v-text-field>
            <v-text-field
                label="本地路径"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-list>
            ...
            >
              <v-list-item-content>
                <v-list-item-title>{{ item.name || item.repo_url }}({{item.desc}})</v-list-item-title>
              </v-list-item-content>
              <v-list-item-action>
                <v-checkbox
        ...
        <v-card-actions>
          <v-btn text color="blue darken-1" @click="closeImportDialog">取消</v-btn>
          <v-spacer></v-spacer>
          <v-btn text color="blue darken-1" @click="confirmImport">确定导入</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    saveRepoDialog() {
      this.$set(this.config.repos, this.selectedRepoIndex, this.selectedRepo)
