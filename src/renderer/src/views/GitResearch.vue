...
export default {
  ...
  data() {
    return {
      ...
      searchTask: null, // 搜索任务的timeout引用
  ...
  methods: {
    ...
    prevMdChunk() {
      if (this.mdCurrentChunk > 0) {
        ...
        // 如果当前有搜索文本，切换块时自动刷新搜索结果
        if (this.mdSearchText && this.mdSearchText.length >= 3 && this.fileContent) {
          this.isSearching = true
          this.asyncSearchInMarkdown(
            this.fileContent.toLowerCase(),
            this.mdSearchText.toLowerCase(),
            true
          )
        }
      }
    ...
    // 异步搜索实现，分批处理避免UI阻塞
    asyncSearchInMarkdown(content, searchText, fromChunkChange = false) {
      // 取消之前的搜索任务（如果存在）
      if (this.searchTask) {
      ...
      const results = []
      const maxResults = 300 // 最大搜索结果数量限制
      ...
      const processChunk = () => {
        // 如果搜索文本已更改，则停止当前搜索
        if (this.mdSearchText.toLowerCase() !== searchText) {
          this.isSearching = false
          return
        }
        ...
        if (results.length >= maxResults) {
          ...
          this.isSearching = false
          console.log(`搜索结果超过${maxResults}个，已自动停止搜索`)
          return
        }
        ...
        while (index < endIndex) {
          ...
          if (foundIndex === -1 || foundIndex >= endIndex) break

          // 计算所在的块索引
          const chunkIndex = Math.floor(foundIndex / this.mdChunkSize)
          results.push({
            globalIndex: foundIndex,
            chunkIndex: chunkIndex,
            localIndex: foundIndex % this.mdChunkSize
          })

          index = foundIndex + searchText.length

          // 如果已达到最大结果数量，提前结束当前循环
          if (results.length >= maxResults) {
            break
          }

          // 如果单次循环处理时间过长，中断并让出主线程
        ...
        // 如果是第一次找到结果且不是块切换触发的搜索，跳转到第一个结果
        if (results.length > 0 && this.mdCurrentSearchIndex === 0 && !fromChunkChange) {
    ...
    highlightMarkdownSearchText(html) {
      ...
      return html.replace(regex, (match, p1) => {
        ...
        const globalResultIndex = this.mdSearchResults.findIndex(
          (result) =>
            result.chunkIndex === this.mdCurrentChunk &&
            currentChunkResults.indexOf(result) === localMatchIndex
        )

        const isCurrentResult =
          globalResultIndex !== -1 && globalResultIndex === this.mdCurrentSearchIndex
        const highlightClass = isCurrentResult ? 'md-search-current' : 'md-search-highlight'
        const result = `<span class="${highlightClass}" data-search-index="${globalResultIndex}">${p1}</span>`
        localMatchIndex++
        return result
      })
    },
    ...
    prevSearchResult() {
      ...
      this.jumpToSearchResult(this.mdCurrentSearchIndex)
    },
    ...
    jumpToSearchResult(resultIndex) {
      if (!this.mdSearchResults[resultIndex]) return

      const result = this.mdSearchResults[resultIndex]

      // 如果是大文件模式，需要切换到对应的块
      if (this.isLargeFile && this.mdTotalChunks > 1) {
        if (this.mdCurrentChunk !== result.chunkIndex) {
          this.mdCurrentChunk = result.chunkIndex
          // 等待块切换完成后再滚动
          this.$nextTick(() => {
            setTimeout(() => {
    ...
    handleSearchKeydown(event) {
      if (!this.mdSearchText || this.mdSearchResults.length === 0) return
...
/* 本地图片样式保持原有逻辑 */
:deep(.markdown-preview) img.local-image-placeholder {
...
:deep(.markdown-preview) img.local-image-placeholder::before {
  content: '正在加载图片...' !important;
...
:deep(.markdown-preview) img.local-image-error {
